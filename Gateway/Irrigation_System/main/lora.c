...
void lora_write_reg(int reg, int val)
{
   ...
   {
      ESP_LOGE("LORA", "Failed to take mutex in lora_write_reg");
   }
   xSemaphoreGive(spi_mutex);
...
int lora_init(void)
{
   ...
   if (spi_mutex == NULL)
   {
      ...
      ESP_LOGE("SPI MUTEX", "Failed to create mutex");
      return 1;
   }
   ...
   {
      // Handle error (mutex creation failed)
      ESP_LOGE("SPI MUTEX", "Failed to create xSpiMutex");
      return 1;
   }
   esp_err_t ret;
...
void lora_send_packet(uint8_t *buf, int size)
{
   ...
   ESP_LOGI("LORA_SEND_PACKET", "Sending packet: %s", (char *)buf_copy);
}
...
void send_lora_packet(const uint8_t *data, int len)
{
   if (xSemaphoreTake(xSpiMutex, portMAX_DELAY) == pdTRUE)
   {
      lora_send_packet(data, len);
...
void receive_lora_packet()
{
   ...
   if (xSemaphoreTake(xSpiMutex, portMAX_DELAY) == pdTRUE)
   {
      ...
      if (lora_received())
      {
         ...
         if (receive_len > 0)
         {
            Publisher_Task("/flespi/qos0", (char *)buf);
            ESP_LOGI(TAG, "%d byte packet received:[%.*s]", receive_len, receive_len, buf);
      ...
      if (received_data)
      {
         // Send back the "200" message
         numberReceive++;
         uint8_t message[4];
         int message_len = sprintf((char *)message, "200");
         send_lora_packet(message, message_len);
         check_send = true;
         received_data = false;
...
void lora_send_data(const uint8_t *data, int len, int node)
{
   send_lora_packet(data, len);
   ...
   if (!check_send)
   {
      ...
      if (i >= 5)
      {
         char *message = (char *)malloc(50 * sizeof(char)); // Allocating memory for message
         if (message == NULL)
         {
         ...
         sprintf(message, "Node %d have problem", node + 1);
         Publisher_Task("alert", message);
         free(message); // Freeing the allocated memory
      }
   }
   check_send = false;
}
