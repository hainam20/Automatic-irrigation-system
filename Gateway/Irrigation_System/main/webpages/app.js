$(document).ready(function () {
  //   getConnectInfo();
  $("#connect_wifi").on("click", function () {
    checkCredentials();
    // alert("Button clicked!");
  });
  $("#disconnect_wifi").on("click", function () {
    disconnectWifi();
  });
  $("#connect_mqtt").on("click", function () {
    checkMQTTCredentials();
    // alert("Button clicked!");
  });
  $("#disconnect_mqtt").on("click", function () {
    disconnectMQTT();
    // alert("Button clicked!");
  });
  $("#NumberNodes").submit(function (event) {
    event.preventDefault();
    numberNodes();
    // alert("Button clicked!");
  });
});

function numberNodes(event) {
  // Get the SSID and password
  // Get the quantity value
  var quantity = $("#quantity").val();

  $.ajax({
    url: "/quantity.json",
    dataType: "json",
    method: "POST",
    contentType: "application/json", // Ensure the content type is set to JSON
    cache: false,
    data: JSON.stringify({
      quantity: quantity,
    }),
    success: function (response) {
      // Handle success
      document.getElementById("quantity_status").innerHTML =
        "<h4 class='gr'>Data sent successfully!</h4>";
    },
    error: function (jqXHR, textStatus, errorThrown) {
      // Handle error
      document.getElementById("quantity_status").innerHTML =
        "<h4 class='gr'>Failed to send data!</h4>";
    },
  });
}
/**
 * Gets the WiFi connection status.
 */
function getWifiConnectStatus() {
  var xhr = new XMLHttpRequest();
  var requestURL = "/wifiConnectStatus";
  xhr.open("POST", requestURL, false);
  xhr.send("wifi_connect_status");

  if (xhr.readyState == 4 && xhr.status == 200) {
    var response = JSON.parse(xhr.responseText);

    document.getElementById("wifi_connect_status").innerHTML = "Connecting...";

    if (response.wifi_connect_status == 2) {
      document.getElementById("wifi_connect_status").innerHTML =
        "<h4 class='rd'>Failed to Connect. Please check your AP credentials and compatibility</h4>";
      stopWifiConnectStatusInterval();
    } else if (response.wifi_connect_status == 3) {
      document.getElementById("wifi_connect_status").innerHTML =
        "<h4 class='gr'>Connection Success!</h4>";
      stopWifiConnectStatusInterval();
    }
  }
}
function stopWifiConnectStatusInterval() {
  if (wifiConnectInterval != null) {
    clearInterval(wifiConnectInterval);
    wifiConnectInterval = null;
  }
}
function startWifiConnectStatusInterval() {
  wifiConnectInterval = setInterval(getWifiConnectStatus, 2800);
}
function connectWifi() {
  // Get the SSID and password
  selectedSSID = $("#connect_ssid").val();
  pwd = $("#connect_pass").val();
  $.ajax({
    url: "/wifiConnect.json",
    dataType: "json",
    method: "POST",
    cache: false,
    // headers: { "my-connect-ssid": selectedSSID, "my-connect-pwd": pwd },
    data: JSON.stringify({
      ssid: selectedSSID,
      password: pwd,
    }),
  });

  startWifiConnectStatusInterval();
}

function checkCredentials() {
  errorList = "";
  credsOk = true;

  selectedSSID = $("#connect_ssid").val();
  pwd = $("#connect_pass").val();
  if (selectedSSID == "") {
    errorList += "<h4 class='rd'>SSID cannot be empty!</h4>";
    credsOk = false;
  }
  if (pwd == "") {
    errorList += "<h4 class='rd'>Password cannot be empty!</h4>";
    credsOk = false;
  }

  if (credsOk == false) {
    $("#wifi_connect_credentials_errors").html(errorList);
  } else {
    $("#wifi_connect_credentials_errors").html("");
    connectWifi();
  }
}
/**
 * Shows the WiFi password if the box is checked.
 */
function showPassword() {
  var x = document.getElementById("connect_pass");
  if (x.type === "password") {
    x.type = "text";
  } else {
    x.type = "password";
  }
}

function disconnectWifi() {
  $.ajax({
    url: "/wifiDisconnect.json",
    dataType: "json",
    method: "DELETE",
    cache: false,
    data: { timestamp: Date.now() },
  });
  // Update the web page
  setTimeout("location.reload(true);", 2000);
}
function disconnectMQTT() {
  $.ajax({
    url: "/mqttDisconnect.json",
    dataType: "json",
    method: "DELETE",
    cache: false,
    data: { timestamp: Date.now() },
  });
  // Update the web page
  setTimeout("location.reload(true);", 2000);
}

function checkMQTTCredentials() {
  errorList = "";
  credsOk = true;

  selectedURI = $("#my-URI").val();
  clientID = $("#my-client-id").val();
  username = $("#my-username").val();
  pwd = $("#my-password").val();
  if (username == "") {
    errorList += "<h4 class='rd'>Username cannot be empty!</h4>";
    credsOk = false;
  }
  if (selectedURI == "") {
    errorList += "<h4 class='rd'>URI cannot be empty!</h4>";
    credsOk = false;
  }
  if (clientID == "") {
    errorList += "<h4 class='rd'>ClientID cannot be empty!</h4>";
    credsOk = false;
  }
  if (credsOk == false) {
    $("#mqtt_connect_credentials_errors").html(errorList);
  } else {
    $("#mqtt_connect_credentials_errors").html("");
    connectMQTT();
  }
}
function connectMQTT() {
  selectedURI = $("#my-URI").val();
  clientID = $("#my-client-id").val();
  username = $("#my-username").val();
  pwd = $("#my-password").val();
  console.log("URI: " + selectedURI);
  console.log("ClientId: " + clientID);
  console.log("Username: " + username);
  console.log("Password: " + pwd);
  $.ajax({
    url: "/mqttConnect.json",
    dataType: "json",
    method: "POST",
    cache: false,
    data: JSON.stringify({
      URI: selectedURI,
      ClientId: clientID,
      Username: username,
      Password: pwd,
    }),
  });
  startMQTConnectStatusInterval();
}

function startMQTConnectStatusInterval() {
  MQTTConnectInterval = setInterval(getMQTTConnectStatus, 2800);
}

function stopMQTTConnectStatusInterval() {
  if (MQTTConnectInterval != null) {
    clearInterval(MQTTConnectInterval);
    MQTTConnectInterval = null;
  }
}
/**
 * Gets the MQTT connection status.
 */
function getMQTTConnectStatus() {
  var xhr = new XMLHttpRequest();
  var requestURL = "/mqttConnectStatus";
  xhr.open("POST", requestURL, false);
  xhr.send("mqtt_connect_status");

  if (xhr.readyState == 4 && xhr.status == 200) {
    var response = JSON.parse(xhr.responseText);

    document.getElementById("mqtt_connect_status").innerHTML = "Connecting...";

    if (response.mqtt_connect_status == 6) {
      document.getElementById("mqtt_connect_status").innerHTML =
        "<h4 class='rd'>Failed to Connect. Please check your MQTT credentials and compatibility</h4>";
      stopMQTTConnectStatusInterval();
    } else if (response.mqtt_connect_status == 7) {
      document.getElementById("mqtt_connect_status").innerHTML =
        "<h4 class='gr'>Connection Success!</h4>";
      stopMQTTConnectStatusInterval();
    }
  }
}
